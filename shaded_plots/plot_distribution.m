...
[X,Y,sigma_value,color_value,alpha_value,line_width] = parseinputs(varargin{:});


MU = mean(Y,1);
if isempty(sigma_value)
    SIGMA = std(Y,[],1);
else
    SIGMA = repmat(sigma_value, [1 size(Y,2)]);
end
...
function [X,Y,sigma_value,color_value,alpha_value,line_width] = parseinputs(varargin)
...
if nargin > minargs
    for i=(minargs+1):2:nargin
        ...
        PNAME = validatestring(PNAME,{'Sigma','Alpha','Color','LineWidth','Prctile'},...
            mfilename,'ParameterName',i);
        
        switch PNAME
            case 'Sigma'
                validateattributes(PVALUE,{'numeric'},{'scalar','nonnan','finite','nonnegative'},mfilename,PNAME,i+1);
                sigma_value = PVALUE;
            case 'Alpha'
